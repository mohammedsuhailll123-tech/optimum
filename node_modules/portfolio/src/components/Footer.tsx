...
export const Footer: React.FC<FooterProps> = ({ currentLang: initialLang }) => {
  ...
  return (
    <footer className="bg-canvas text-body border-t border-glass-border select-none relative pt-16 pb-8">
      {/* Top running gradient border indicator */}
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 mb-12">
        ...
        <div className="lg:col-span-4 flex flex-col items-start text-left rtl:text-right">
          <a 
            ...
            onClick={scrollToTop}
          >
            <div className="relative w-14 h-14 rounded-xl bg-gradient-to-tr from-accent-blue to-accent-cyan flex items-center justify-center shadow-lg overflow-hidden">
              <img src="/globe_logo.png" className="w-full h-full object-contain" alt="Optimum Supply Logo" />
            </div>
            <span className="font-sans font-extrabold text-base tracking-tight text-ink uppercase">
              {lang === 'ar' ? (
                <>أوبتيموم <span className="text-accent-cyan font-bold">للتوريد</span></>
              ) : (
                <>OPTIMUM <span className="text-accent-blue font-bold">SUPPLY</span></>
              )}
            </span>
          </a>
        ...
        {/* Links Column 1: Product categories */}
        <div className="lg:col-span-2 text-left rtl:text-right">
          ...
          <ul className="space-y-2.5 text-xs font-semibold">
            {[
              { href: '#products', label: lang === 'ar' ? 'التوريدات الصناعية' : 'Industrial Supplies' },
              { href: '#products', label: lang === 'ar' ? 'المنتجات الميكانيكية' : 'Mechanical Products' },
              { href: '#products', label: lang === 'ar' ? 'المعدات الكهربائية' : 'Electrical Supplies' },
              { href: '#products', label: lang === 'ar' ? 'أدوات ومعدات اللحام' : 'Welding Equipment' },
              { href: '#products', label: lang === 'ar' ? 'معدات السلامة المهنية' : 'Safety Equipment' }
            ].map((link, idx) => (
              <li key={idx}>
        ...
        {/* Links Column 2: Quick Links mapping */}
        <div className="lg:col-span-2 text-left rtl:text-right">
          <h4 className="caption-mono text-ink mb-4 font-bold">
            {t.quickLinks}
          </h4>
          <ul className="space-y-2.5 text-xs font-semibold">
            {[
              ...
              { href: '#products', label: lang === 'ar' ? 'كتالوج المنتجات' : 'Product Catalogue' },
              { href: '#industries', label: lang === 'ar' ? 'القطاعات الصناعية' : 'Sectors Served' },
              { href: '#why-us', label: lang === 'ar' ? 'ميزاتنا التنافسية' : 'Why Choose Us' },
        ...
        <div className="lg:col-span-4 text-left rtl:text-right">
          ...
          <p className="text-xs text-body leading-relaxed mb-4">
            {lang === 'ar'
              ? 'اشترك في نشرتنا لتلقي نشرات التوريد وتوفر المواد والمعدات الهندسية.'
              : 'Subscribe to receive product bulletins, catalog updates, and regional sourcing news.'}
          </p>
